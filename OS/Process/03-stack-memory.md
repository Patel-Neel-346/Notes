...
### Recursion in Real Assembly: The Return Address and Instruction Cache Reuse

The prologue/epilogue shown above (`push rbp` / `mov rbp,rsp` / `sub rsp,N` ... `leave` / `ret`) is the general x86-64 pattern. `call` and `ret` are the two instructions that actually move the return address around: `call target` pushes the address of the *next* instruction onto the stack and jumps to `target`; `ret` pops that address back off the stack into the instruction pointer. Unlike ARM (which has a dedicated Link Register), x86-64 always writes the return address straight to the stack — there's no separate register to hold it temporarily.

This becomes very visible in a **recursive** function, where the same block of code calls itself repeatedly:
...
Here is what `gcc -O0` actually produces for this, shown as a disassembly (address, then instruction — this is the same format `objdump -d` prints):

```
foo:
    0x1149   push  rbp                  ; save caller's frame pointer
    0x114a   mov   rbp, rsp             ; anchor this frame
    0x114d   sub   rsp, 16              ; space for local copy of 'a'
    0x1151   mov   [rbp-4], edi         ; store parameter a
    0x1154   cmp   dword [rbp-4], 0     ; compare a to 0
    0x1158   jg    0x1170               ; if a > 0, skip to base case
    0x115a   mov   eax, [rbp-4]         ; eax = a
    0x115d   add   eax, 1               ; eax = a + 1
    0x1160   mov   edi, eax             ; set up argument for the call
    0x1163   call  0x1149               ; RECURSE: push return addr 0x1168, jump to foo
    0x1168   jmp   0x1174               ; <- execution resumes HERE after the call returns
    0x1170   mov   eax, [rbp-4]         ; base case: eax = a
    0x1174   leave                      ; SP = BP, pop caller's BP
    0x1175   ret                        ; pop return address into instruction pointer

main:
    0x1180   push  rbp
    0x1181   mov   rbp, rsp
    0x1184   mov   edi, -1              ; argument: a = -1
    0x1189   call  0x1149               ; call foo(-1): push return addr 0x118e
    0x118e   mov   ecx, eax             ; <- execution resumes HERE, result now in eax
    0x1190   mov   eax, 0
    0x1193   pop   rbp
    0x1194   ret
```

> [!important] Every recursive call re-runs the exact same 0x2c bytes
> `foo` calling itself doesn't create new code anywhere — the instruction pointer simply jumps back to `0x1149` again and again, re-executing the identical bytes with a different value of `a` sitting on the stack each time. `foo`'s entire body spans `0x1149` to `0x1175` — only 44 bytes, comfortably inside a single 64-byte cache line. That means **only the very first fetch of `foo`, the first time it's ever called in the program, is a genuine ~100 ns memory access.** Every recursive re-entry after that — no matter how deep the recursion goes — is an L1 cache hit at roughly 1–2 ns, because those exact bytes are already sitting hot in cache from the previous call.

**Step-by-step trace** for `foo(-1)`:

| Step | Address | Instruction | What happens | Stack (top frame) | I-cache |
|---|---|---|---|---|---|
| 1 | `0x1189` | `call 0x1149` | `main` pushes return address `0x118e`, jumps to `foo` | `main` frame | HIT (main's block already warm) |
| 2 | `0x1149` | `push rbp` | Enter `foo(a=-1)` — first-ever fetch of `foo`'s code | new `foo` frame, `a=-1` | **MISS (~100 ns)** — cold |
| 3 | `0x1154`–`0x1158` | `cmp` / `jg` | `-1 > 0` is false → falls through, will recurse | `foo(a=-1)` | HIT |
| 4 | `0x1163` | `call 0x1149` | Pushes return address `0x1168`, recurses into `foo(a=0)` | new `foo` frame, `a=0` | HIT — same 44-byte block, already cached |
| 5 | `0x1154`–`0x1158` | `cmp` / `jg` | `0 > 0` is false → recurse again | `foo(a=0)` | HIT |
| 6 | `0x1163` | `call 0x1149` | Pushes return address `0x1168`, recurses into `foo(a=1)` | new `foo` frame, `a=1` | HIT |
| 7 | `0x1158` | `jg 0x1170` | `1 > 0` is true → **base case**, jumps straight to `0x1170`, skipping the `call` entirely | `foo(a=1)` | HIT |
| 8 | `0x1175` | `ret` | Pops return address `0x1168` → resumes in `foo(a=0)` at `0x1168` | frame popped, back to `foo(a=0)` | HIT |
| 9 | `0x1175` | `ret` | Pops return address `0x1168` → resumes in `foo(a=-1)` at `0x1168` | frame popped, back to `foo(a=-1)` | HIT |
| 10 | `0x1175` | `ret` | Pops return address `0x118e` → resumes in `main` at `0x118e`, result in `eax` | frame popped, back to `main` | HIT |

Notice that every unwind step (8, 9) lands on the exact same address, `0x1168` — because every recursive call originated from the exact same `call` instruction inside `foo` itself. Only the *outermost* call, from `main`, returns to a different address (`0x118e`).

> [!tip] See real recursion assembly yourself
> ```sh
> echo 'int foo(int a){ if(a<=0) return foo(a+1); return a; }' > foo.c
> gcc -O0 -c foo.c && objdump -d -M intel foo.o
> ```
...
Recursion grows the stack — one new frame per call. A **loop** does the opposite: it reuses one single frame and just mutates the values inside it, jumping backward to re-run the same instructions.
...
sum_to_n:
    0x11a0   push  rbp
    0x11a1   mov   rbp, rsp
    0x11a4   mov   [rbp-20], edi       ; store parameter n
    0x11a7   mov   dword [rbp-4], 0    ; total = 0
    0x11ae   mov   dword [rbp-8], 1    ; i = 1
.loop_check:
    0x11b5   mov   eax, [rbp-8]        ; eax = i        <- loop re-enters HERE
    0x11b8   cmp   eax, [rbp-20]       ; compare i to n
    0x11bb   jg    0x11d5              ; if i > n, exit the loop
    0x11bd   mov   eax, [rbp-4]        ; eax = total
    0x11c0   add   eax, [rbp-8]        ; eax = total + i
    0x11c3   mov   [rbp-4], eax        ; total = eax
    0x11c6   mov   eax, [rbp-8]        ; eax = i
    0x11c9   add   eax, 1              ; eax = i + 1
    0x11cc   mov   [rbp-8], eax        ; i = eax
    0x11cf   jmp   0x11b5              ; jump BACKWARD to .loop_check
.loop_end:
    0x11d5   mov   eax, [rbp-4]        ; return total
    0x11d8   pop   rbp
    0x11d9   ret
```

**Step-by-step trace** for `sum_to_n(3)`:

| Step | Address | Instruction | `total` | `i` | I-cache |
|---|---|---|---|---|---|
| 1 | `0x11a0`–`0x11ae` | prologue, init | 0 | 1 | first fetch → MISS, rest of prologue → HIT |
| 2 | `0x11b5`–`0x11bb` | check `1 <= 3` → true | 0 | 1 | MISS (first time reaching the loop body), then HIT |
| 3 | `0x11bd`–`0x11cf` | `total += i`, `i++`, jump back | 1 | 2 | HIT |
| 4 | `0x11b5`–`0x11bb` | check `2 <= 3` → true | 1 | 2 | **HIT** — same address as step 2 |
| 5 | `0x11bd`–`0x11cf` | `total += i`, `i++`, jump back | 3 | 3 | HIT |
| 6 | `0x11b5`–`0x11bb` | check `3 <= 3` → true | 3 | 3 | HIT |
| 7 | `0x11bd`–`0x11cf` | `total += i`, `i++`, jump back | 6 | 4 | HIT |
| 8 | `0x11b5`–`0x11bb` | check `4 <= 3` → false, exit | 6 | 4 | HIT |
| 9 | `0x11d5`–`0x11d9` | return `total` (6) | 6 | 4 | HIT |

> [!important] The stack does not grow with loop iterations
> Compare this directly to the recursion trace above. In recursion, `call` pushed a brand-new frame on every level — 3 levels of recursion meant 3 frames alive at once, using `O(depth)` stack space. Here, `total` and `i` live at fixed offsets (`rbp-4`, `rbp-8`) in **one single frame** for the entire function — the loop just overwrites those same two memory locations on every pass. That's why an iterative version of a function almost always uses less stack space than a recursive one solving the same problem, and why converting deep recursion to a loop is a standard fix for stack-overflow-prone code.
>
> The cache story is even more one-sided than recursion: the entire loop body (`0x11b5`–`0x11cf`, just 26 bytes) is fetched *backward* on every single iteration, yet — because it's the same tiny address range every time — only the very first pass through it ever pays the ~100 ns miss. A loop running a million iterations pays that cost exactly once.
...
* Recursive call trace: `call`/`ret` addresses and L1 cache hit/miss across repeated calls to the same code
* Loop trace: backward `jmp` re-entering the same address range each iteration, one flat frame throughout
